...
static ImageRec *ImageOpen(char *fileName)
{
    ...
    if (fread(image, 1, 12, image->file) == -1) {
		printf("FAILURE IN texload\n");
	}
    ...
    if ((image->type & 0xFF00) == 0x0100) {
        ...
        if (fread(image->rowStart, 1, x, image->file) == -1) {
			printf("FAILURE2 in texload\n");
		}
        if (fread(image->rowSize, 1, x, image->file) == -1) {
			printf("FAILURE3 in texload\n");
		}
        if (swapFlag) {
...
ImageGetRow(ImageRec *image, unsigned char *buf, int y, int z) {
    ...
    if ((image->type & 0xFF00) == 0x0100) {
        ...
        if (fread(image->tmp, 1, (unsigned int)image->rowSize[y+z*image->ysize],
              image->file) == -1) {
				  printf("FAILURE4 in texload\n");
		}
        ...
        fseek(image->file, 512+(y*image->xsize)+(z*image->xsize*image->ysize),
              SEEK_SET);
        if (fread(buf, 1, image->xsize, image->file) !=1) {
			printf("FAILURE5 in texload\n");
		}
	}
}
